...
const Chatbox = () => {
  ...
  const [availableRooms, setAvailableRooms] = useState<{ name: string; type: 'chat' | 'game' }[]>([]);
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={() => createRoom({ newRoomName: 'New Chat Room', type: 'chat' })}>Create Chat Room</button>
        <button onClick={() => createRoom({ newRoomName: 'New Game Room', type: 'game' })}>Create Game Room</button>
      </div>
    </div>
  );
};
